...
void lar_solver::solve_with_core_solver() {
    ...
    lp_assert((m_int_solver->m_number_of_calls % 100) != 0 || m_status != lp_status::OPTIMAL || all_constraints_hold());
}
...
void lar_solver::add_row_from_term_no_constraint(const lar_term * term, unsigned term_ext_index) {
    ...
    m_mpq_lar_core_solver.m_r_solver.update_x_and_do_not_track_feasibility(j, get_basic_var_value_from_row_directly(A_r().row_count() - 1));
    if (use_lu())
	...
	for (unsigned j = 0; j < column_count(); j++) {
		...
		if (del < -impq(mpq(1, 2))) {
			...
			v = flv;
                ...
                m_cube_rounded_columns.insert(j);
