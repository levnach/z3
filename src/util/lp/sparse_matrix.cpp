...
template <typename T, typename X>
void sparse_matrix<T, X>::copy_B(static_matrix<T, X> const &A, std::vector<unsigned> & basis) {
...
template <typename T, typename X>
sparse_matrix<T, X>::sparse_matrix(static_matrix<T, X> const &A, std::vector<unsigned> & basis) :
...
template <typename T, typename X>
void sparse_matrix<T, X>::remove_element(std::vector<indexed_value<T>> & row_vals, unsigned row_offset, std::vector<indexed_value<T>> & column_vals, unsigned column_offset) {
...
template <typename T, typename X>
void sparse_matrix<T, X>::remove_element(std::vector<indexed_value<T>> & row_chunk, indexed_value<T> & row_el_iv) {
...
template <typename T, typename X>
void sparse_matrix<T, X>::put_max_index_to_0(std::vector<indexed_value<T>> & row_vals, unsigned max_index)  {
...
template <typename T, typename X>
void sparse_matrix<T, X>::set_max_in_row(std::vector<indexed_value<T>> & row_vals) {
    if (row_vals.size() == 0)
        return;
...
template <typename T, typename X>
void sparse_matrix<T, X>::scan_row_to_work_vector(unsigned row, unsigned pivot_column) {
...
template <typename T, typename X>
bool sparse_matrix<T, X>::pivot_row_to_row(unsigned i, T alpha, unsigned i0, lp_settings & settings ) {
...
// returns false if the resulting row is all zeroes, and true otherwise
template <typename T, typename X>
bool sparse_matrix<T, X>::set_row_from_work_vector_and_clean_work_vector(unsigned i0) {
...
template <typename T, typename X>
void sparse_matrix<T, X>::divide_row_by_constant(unsigned i, T & t, lp_settings & settings) {
...
template <typename T, typename X>
void sparse_matrix<T, X>::solve_y_U(std::vector<T> & y) const { // works by rows
#ifdef LEAN_DEBUG
...
template <typename T, typename X>
template <typename L>
void sparse_matrix<T, X>::double_solve_U_y(std::vector<L>& y){
...
template <typename T, typename X>
template <typename L>
void sparse_matrix<T, X>::solve_U_y(std::vector<L> & y) { // it is a column wise version
#ifdef LEAN_DEBUG
    // T * rs = clone_vector<T>(y, dimension());
#endif
...
template <typename T, typename X>
template <typename L>
L sparse_matrix<T, X>::dot_product_with_row (unsigned row, const std::vector<L> & y) const {
...
template <typename T, typename X>
void sparse_matrix<T, X>::remove_element_that_is_not_in_w(std::vector<indexed_value<T>> & column_vals, indexed_value<T> & col_el_iv) {
...
template <typename T, typename X>
void sparse_matrix<T, X>::recover_pivot_queue(std::vector<upair> & rejected_pivots)  {
